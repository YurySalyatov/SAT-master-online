...
from SmipleGNN.utils import normalize_adj
...
parser = argparse.ArgumentParser(description='Evaluate the classification performance with only A')
parser.add_argument('--mode', type=str, default='A', choices=['A', 'AX'],
                    help='The mode of file')
parser.add_argument('--method_name_A', type=str, default='GCN', choices=['GCN', 'DeepWalk'],
                    help='The name of the method GCN or DeepWalk')
parser.add_argument('--dataset_A', type=str, default='core', choices=['cora', 'citeseer', 'plumed', 'ms_academic'],
                    help='The name of the dataset for only A evaluation')
parser.add_argument('--method_name_AX', type=str, default='GCN', choices=['NeighAggre', 'GCN', 'VAE', 'LFI', 'GAT'],
                    help='The name of the method GCN or DeepWalk')
parser.add_argument('--dataset_AX', type=str, default='cora', choices=['cora', 'citeseer', 'pubmed'],
                    help='The name of the dataset for only AX evaluation')
...
if mode == 'A':
    ...
    '''
    method_name = args.method_name_A  # DeepWalk, GCN
    dataset = args.dataset_A  # cora, citeseer, pubmed, ms_academic
    train_fts_ratio = 0.4
    ...
    # os.environ['CUDA_VISIBLE_DEVICES'] = ' '
    # set necessary parameters
    method_name = args.method_name_AX # NeighAggre, GCN, VAE, LFI, GAT
    dataset = args.dataset_AX # cora, citeseer, pubmed
    ...
    if method_name == 'LFI':
        gene_fts = pickle.load(open(os.path.join(os.getcwd(), 'features', method_name,
                                                 'gene_fts_train_ratio_{}_{}_G1.0_R1.0_C{}.pkl'.format(dataset,
                                                                                                       train_fts_ratio, c)),
                                    'rb'))
    else:
        gene_fts = pickle.load(open(os.path.join(os.getcwd(), 'features', method_name,
