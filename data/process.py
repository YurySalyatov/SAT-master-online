...
if dataset == 'steam':
    ...
    if not os.path.exists(os.path.join(os.getcwd(), dataset, 'freq_item_mat.pkl')):
        itemID_userID_dict = {}
        with open(os.path.join(os.getcwd(), dataset, 'user_product_date.csv'), 'rb', encoding='utf-8') as f:
            lines = f.readlines()[1:]
    ...
    itemID_tagID_dict = {}
    with open(os.path.join(os.getcwd(), dataset, 'product_tags.csv'), 'r', encoding='utf-8') as f:
        lines = f.readlines()[1:]
        for i in tqdm(range(len(lines))):
