...
diag_fts, adj, train_fts, val_fts = Variable(features), Variable(adj), Variable(true_features[train_fts_idx]), Variable(
    true_features[vali_fts_idx])
...
def compute_test():
    ...
    pickle.dump(save_fts, open(os.path.join(os.getcwd(), 'features', method_name,
                                            'gene_fts_train_ratio_{}_{}.pkl'.format(args.dataset, train_fts_ratio)),
                               'wb'))
...
model.load_state_dict(torch.load(
    os.path.join(os.getcwd(), 'output', method_name, 'best_gcn_{}_{}.pkl'.format(args.dataset, train_fts_ratio))))
